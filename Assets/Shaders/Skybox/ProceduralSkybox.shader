...
Properties {
	...
	_SunDirection ("Sun Direction", Vector) = (.577, 0.577, 0.577, 0)
...
//				for(int i=0; i<int(kSamples); i++)
				{
					...
					float lightAngle = dot(_SunDirection.xyz, samplePoint) / height;
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
			...
			OUT.skyColor	= _Exposure * (cIn * getRayleighPhase(_SunDirection.xyz, -eyeRay));
		...
		// if y < 0 [eyeRay.y > 0] - sky
			...
			if(y < 0.0)
			{
				...
				half mie = calcSunSpot(_SunDirection, -ray);
			#else // SKYBOX_SUNDISK_HQ
				half eyeCos = dot(_SunDirection, ray);
