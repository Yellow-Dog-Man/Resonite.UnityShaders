...
half4 calcMetallicSmoothness(XSLighting i)
{
	...
	gloss *= i.metallicGlossMap.a;
	metallic *= i.metallicGlossMap.r;
#endif

    half roughness = 1- gloss;
    roughness *= 1.7 - 0.7 * roughness;

    //metallic *= i.reflectivityMask.r;
...
half4 calcOutlineColor(XSLighting i, DotProducts d, half3 indirectDiffuse, half4 lightCol)
{
    ...
    #if defined(Geometry)
		half3 ol = _OutlineColor;

//#ifdef OUTLINE_ALBEDO_TINT
		UNITY_BRANCH
		if (_OutlineAlbedoTint)
			ol *= i.diffuseColor;
//#endif
...
half4 calcRamp(XSLighting i, DotProducts d)
{
	...
	#ifdef RAMP_MASK
	half2 uv = half2(remapRamp, i.rampMask.r);
	#else
	half2 uv = half2(remapRamp, 0);
	#endif

    half4 ramp = tex2D( _Ramp, uv);
