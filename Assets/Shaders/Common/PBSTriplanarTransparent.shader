...
Shader "PBSTriplanarTransparent"
{
	Properties
	{
	...
	#ifdef _METALLICMAP
			fixed4 metalX = tex2D(_MetallicMap, uvX);
			fixed4 metalY = tex2D(_MetallicMap, uvY);
			fixed4 metalZ = tex2D(_MetallicMap, uvZ);
			fixed4 metal = (metalX * triblend.x + metalY * triblend.y + metalZ * triblend.z);
