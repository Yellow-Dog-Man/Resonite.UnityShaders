Shader "PBSColorSplat"
{
	Properties
	{
		...
		_NormalScale("Normal Scale 0", Float) = 1
		...
		_MetallicGloss01("Metallic Gloss 01", 2D) = "black" {}
		_MetallicGloss23("Metallic Gloss 23", 2D) = "black" {}
		...
		_Metallic("Metallic 0", Range(0, 1)) = 0
		...
		SubShader
		{
	...
	#ifdef _METALLICMAP
			sampler2D _MetallicGloss01;
			sampler2D _MetallicGloss23;
#endif
			float _Glossiness;
			float _Glossiness1;
			float _Glossiness2;
			...
			void surf(Input IN, inout SurfaceOutputStandard o)
			{
	...
	#ifdef _METALLICMAP
				fixed4 m01 = tex2D(_MetallicGloss01, IN.uv_Albedo);
				fixed4 m23 = tex2D(_MetallicGloss23, IN.uv_Albedo);
				...
				m0 *= fixed2(_Metallic, _Glossiness);
				m1 *= fixed2(_Metallic1, _Glossiness1);
				m2 *= fixed2(_Metallic2, _Glossiness2);
				m3 *= fixed2(_Metallic3, _Glossiness3);
	#else
				fixed2 m0 = fixed2(_Metallic, _Glossiness);
				fixed2 m1 = fixed2(_Metallic1, _Glossiness1);
				fixed2 m2 = fixed2(_Metallic2, _Glossiness2);
				fixed2 m3 = fixed2(_Metallic3, _Glossiness3);
	#endif

				fixed2 m = m0 * map.x + m1 * map.y + m2 * map.z + m3 * map.w;
